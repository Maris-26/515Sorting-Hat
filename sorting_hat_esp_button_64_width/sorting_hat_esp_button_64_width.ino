...
// Define button pins
int buttonPins[] = {14, 27, 26, 25};  // Button A, Button B, Button C, Button D

// Debounce delay time (in milliseconds)
unsigned long debounceDelay = 50;  // Time for debounce

// Flag to track if a button has been pressed
bool buttonPressed = false;  // This flag is used to ensure only one button is pressed at a time
...
void checkButtons() {
    ...
    for (int i = 0; i < 4; i++) {
        buttonState = digitalRead(buttonPins[i]); // Read button state

        // If button state has changed, reset debounce timer
        if (buttonState != lastButtonState) {
            lastDebounceTime = millis();  // Record the time when state changes
        }

        // If button is pressed and debounce time has passed
        if ((millis() - lastDebounceTime) > debounceDelay) {
            // If button is pressed (LOW means button is pressed due to INPUT_PULLUP)
            if (buttonState == LOW) {
                responses[questionIndex] = i + 1;  // Store the current answer (1-4)
                buttonPressed = true;  // Set the button press flag
                break;  // Exit loop, only one button can be pressed
            }
        }
    ...
    // If a button was pressed and debounce successful, move to next question
    if (buttonPressed) {
        nextQuestion();
    }
}
